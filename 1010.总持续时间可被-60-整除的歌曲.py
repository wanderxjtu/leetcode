...
class Solution:
    def numPairsDivisibleBy60(self, time: List[int]) -> int:
        if len(time) == 1:
            return 0
        pairs = [0] * 60
        for k, g in groupby(time, lambda x: x % 60):
            pairs[k] += len(list(g))

        count = (
            sum(mul(x, y) for x, y in zip(pairs[1:30], pairs[59:30:-1]))
            + int(pairs[30] * (pairs[30] - 1) / 2)
            + int(pairs[0] * (pairs[0] - 1) / 2)
        )
...
if __name__ == "__main__":
    ...
    print(Solution().numPairsDivisibleBy60([30]))
